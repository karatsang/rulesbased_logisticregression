...
ecoli_ast = pd.read_csv('/Users/karatsang/Documents/repos/karakts_github/AMRgenotypephenotype/ecoli_rgi/Ecoli_ast.tsv', sep='\t').set_index('Sample').replace(SR_encoder)
ecoli_rgi = pd.read_pickle('/Users/karatsang/Documents/repos/karakts_github/AMRgenotypephenotype/ecoli_rgi/rgi_encoded.pkl')
pseudo_ast = pd.read_csv('/Users/karatsang/Documents/repos/karakts_github/AMRgenotypephenotype/paeruginosa_rgi/Paeruginosa_ast.tsv', sep='\t').set_index('Sample').replace(SR_encoder).drop('ertapenem', axis=1)
pseudo_rgi = pd.read_pickle('/Users/karatsang/Documents/repos/karakts_github/AMRgenotypephenotype/paeruginosa_rgi/rgi_encoded.pkl')
...
def fit_LR_model_per_abx(rgi, ast):
    ...
    for abx in ast.columns:        
        ...
        # get p-values
        x_test2 = statsmod.add_constant(x_test)
        est = statsmod.OLS(y_test, x_test2)
        est2 = est.fit()
        # print(est2.summary())
...
def plot_feature_importances(classifiers, rgi):
    ...
    for ix, abx in enumerate(feature_importance):
        # feature importance to excel spreadsheet
        # feature_importance.to_excel("Weights.xlsx",index=True)
...
def over_under_pred_tally(ast, rgi, classifier_dict):
    ...
    data = {'antibiotic': [],
            'classifier': [],
            'Complete R Prediction': [],
            'Complete S Prediction': [],
            'Over Prediction': [],
            'Under Prediction': []}
    ...
    for abx in ast:
        ...
        completeR, under, over, completeS = 0, 0, 0, 0
        for ix, ast_pred in enumerate(ast_pred):
            if ast_pred == 1 and abx_ast[ix] == 1:
                ...
                over += 1
            elif ast_pred == 0 and abx_ast[ix] == 0:
                completeS += 1

        
        data['antibiotic'].append(abx.capitalize())
        data['classifier'].append(classifier_dict[abx])
        data['Complete R Prediction'].append(completeR)
        data['Complete S Prediction'].append(completeS)
        ...
        if add_ert and abx.startswith('cefoxitin'):
            ...
            data['Complete R Prediction'].append(0)
            data['Complete S Prediction'].append(0)
